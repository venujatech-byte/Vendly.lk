...
export default function ShopSaleFilters({ onChange }) {
  ...
  return <section className="shop-sale-filters filter-panel">
    ...
    <div className={`shop-sale-filters__fields filter-panel__form ${open ? "is-open" : ""}`}>
      <label className="filter-panel__field filter-panel__field--search filter-panel__icon-field"><Search size={17} /><input type="search" value={filters.search} onChange={(e) => update("search", e.target.value)} placeholder="Search sale number, item or customer..." /></label>
      ...
      <button className="filter-panel__reset filter-panel__reset--text" type="button" onClick={reset}><RotateCcw size={17} /> Reset</button>
    </div>
  </section>;
}
