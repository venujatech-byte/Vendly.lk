    ...
    <div className="orders-table__scroll"><table className="orders-table"><thead><tr>
      ...
      {pagination.pageItems.map((sale) => <Fragment key={sale.id}>
        <tr><td><button className="orders-table__expand-button" type="button" onClick={() => setExpanded(expanded === sale.id ? null : sale.id)}>{expanded === sale.id ? <ChevronDown size={18}/> : <ChevronRight size={18}/>}</button></td>
          ...
          <td><ActionMenu items={[{ label: "Print receipt", icon: <Printer size={16}/>, onClick: () => onPrint(sale) }, ...((sale.items?.length) ? [{ label: "Warranty claim", icon: <ShieldCheck size={16}/>, onClick: () => onWarranty(sale) }] : []), { label: "Delete sale", icon: <Trash2 size={16}/>, danger: true, onClick: () => onRemove(sale) }]}/></td>
        </tr>
        {expanded === sale.id && <tr className="orders-table__details-row"><td colSpan={6}><div className="shop-sale-details">
          ...
          <section className="shop-sale-details__actions"><button type="button" onClick={() => onPrint(sale)}><Printer size={16}/>Print receipt</button>{(sale.items?.length > 0) && <button type="button" onClick={() => onWarranty(sale)}><ShieldCheck size={16}/>Warranty claim</button>}</section>
        </div></td></tr>}
      </Fragment>)}
