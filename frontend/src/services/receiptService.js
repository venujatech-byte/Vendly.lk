...
export function downloadReceiptPdf(business, order) {
  try {
    ...
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(12);
    const orderNum = order.orderNumber || order.id || order.orderId || "NEW";
    pdf.text(`#${safeText(orderNum)}`, left + 6, y + 13);
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(8);
    ...
    if (order.createdAt) {
      ...
      } else if (order.createdAt.seconds || order.createdAt._seconds) {
        dateObj = new Date((order.createdAt.seconds || order.createdAt._seconds) * 1000);
      } else {
        dateObj = new Date(order.createdAt);
      }
    }
    const dateStr = Number.isNaN(dateObj.getTime()) ? new Date().toLocaleDateString("en-LK") : dateObj.toLocaleDateString("en-LK");
    pdf.text(dateStr, right - 6, y + 13, { align: "right" });
    y += 27;
    ...
    const address = order.deliveryAddress || order.deliveryAddressObject || (order.customerSnapshot && order.customerSnapshot.address) || {};
    ...
    const rows = [["Subtotal", order.subtotalMinor || 0], ["Discount", -Number(order.discountTotalMinor || 0)], ["Delivery", order.deliveryFeeMinor || 0], ["Tax", order.taxTotalMinor || 0]];
    ...
    pdf.setFont("helvetica", "bold"); pdf.setFontSize(12); pdf.text("Total", left + 7, y + 39); pdf.setTextColor(8, 115, 151); pdf.text(amount(order.totalAmountMinor || 0), right - 7, y + 39, { align: "right" });
    const filename = `${safeText(order.orderNumber || order.id || "vendly-order")}-receipt.pdf`;
    pdf.save(filename);
  } catch (err) {
